...
void (*ButtonThreeTask)(void);
void (*ButtonFourTask)(void);
...
//#define BUTTON3		(*((volatile uint32_t *)0x40025040))	/* PF4 */	
//#define BUTTON4		(*((volatile uint32_t *)0x40025004))	/* PF0 */
volatile static uint32_t Last1,Last2,Last3,Last4;
...
//void ButtonThreeInit(uint8_t priority){
//  SYSCTL_RCGCGPIO_R |= 0x00000020; // 1) activate clock for Port F
//  while((SYSCTL_PRGPIO_R&0x20) == 0){};// allow time for clock to stabilize
////	GPIO_PORTF_LOCK_R = 0x4C4F434B;  // 2) unlock GPIO Port F		//only this value unlocks the register. all others lock it. whack. 
////  GPIO_PORTF_CR_R = 0xFF;          // allow changes to PF7-0
//  GPIO_PORTF_AMSEL_R &= ~0x10;     // 3) disable analog on PF4
//                                   // 4) configure PF4 as GPIO
//  GPIO_PORTF_PCTL_R = (GPIO_PORTD_PCTL_R&0xF0FFFFFF)+0x00000000;
...
//	GPIO_PORTF_ICR_R = 0x10;      // (e) clear flag6
...
//void ButtonFourInit(uint8_t priority){
//  SYSCTL_RCGCGPIO_R |= 0x00000020; // 1) activate clock for Port F
//  while((SYSCTL_PRGPIO_R&0x20) == 0){};// allow time for clock to stabilize
//  GPIO_PORTF_AMSEL_R &= ~0x00;     // 3) disable analog on PF0
//                                   // 4) bruh
//  GPIO_PORTF_PCTL_R = (GPIO_PORTD_PCTL_R&0x0FFFFFFF)+0x00000000;
//  GPIO_PORTF_DIR_R &= ~0x00;       // 5) make PF0 input
//  GPIO_PORTF_AFSEL_R &= ~0x00;     // 6) disable alt funct on PF0
//	GPIO_PORTF_DEN_R |= 0x00;        // 7) enable digital I/O on PF0
//	GPIO_PORTF_PUR_R |= 0x00;     //     enable weak pull-up on PF0
//  GPIO_PORTF_IS_R &= ~0x00;     // (d) PF0 is edge-sensitive
//  GPIO_PORTF_IBE_R |= 0x00;     //     PF0 is both edges
//	GPIO_PORTF_ICR_R = 0x00;      // (e) clear flag7
//  GPIO_PORTF_IM_R |= 0x00;      // (f) arm interrupt on PF0 *** No IME bit as mentioned in Book ***
//	
//  NVIC_PRI0_R = (NVIC_PRI0_R&0x1FFFFFFF)|(priority << 29); // (g) priority 2
//  NVIC_EN0_R = 0x00000008;      // (h) enable interrupt 3 in NVIC  
//	Last4 = BUTTON4;
...
void GPIOPortD_Handler(void) {  // called on touch of either B1 or B2
...
//void GPIOPortF_Handler(void) {  // called on touch of either S1 or S2

//	if(GPIO_PORTF_RIS_R & 0x10){   // SWITCH1 touched
...
//	}
//	else if(GPIO_PORTF_RIS_R & 0x00){  // SWITCH2 touched
//		GPIO_PORTF_IM_R &= ~0x00;  //disarm interrupt on PF0
//		if (Last4){
//			(*ButtonFourTask)();
//		}
...
// This task can call OS_Signal  OS_bSignal	 OS_AddThread
// This task does not have a Thread ID
int OS_AddB1Task(void(*task)(void), unsigned long priority) { 
	ButtonOneTask = task;
...
// This task does not have a Thread ID
int OS_AddB2Task(void(*task)(void), unsigned long priority) { 
	ButtonTwoTask = task;
...
//******** OS_AddS1Task *************** 
// add a background task to run whenever the SWITCH1 (PF4) button is pushed
// Inputs: pointer to a void/void background function
//         priority 0 is the highest, 5 is the lowest
// Outputs: 1 if successful, 0 if this thread can not be added
// It is assumed that the user task will run to completion and return
...
// This task can call OS_Signal  OS_bSignal	 OS_AddThread
// This task does not have a Thread ID
//int OS_AddB3Task(void(*task)(void), unsigned long priority) { 
//	ButtonThreeTask = task;
//	ButtonThreeInit(priority);
//	return 1;
//}

////******** OS_AddS2Task *************** 
//// add a background task to run whenever the SWITCH2 (PF0) button is pushed
//// Inputs: pointer to a void/void background function
////         priority 0 is highest, 5 is lowest
//// Outputs: 1 if successful, 0 if this thread can not be added
//// It is assumed user task will run to completion and return
...
//// This task can call issue OS_Signal, it can call OS_AddThread
//// This task does not have a Thread ID
//int OS_AddB4Task(void(*task)(void), unsigned long priority) { 
//	ButtonFourTask = task;
//	ButtonFourInit(priority);
//	return 1;
//}
